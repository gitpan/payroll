...
package Payroll::US::MO::StateIncome;
use strict;
...
sub calculate
{
    ...
    {  $self->setError("Invalid gross: $gross\n"); return undef; }
    if (! $self->isValid(allowances => $allowances) )
    {  $self->setError("Invalid allowances: $allowances\n"); return undef; }
    if (! $self->isValid(marital => $marital) )
    {  $self->setError("Invalid marital: $marital\n"); return undef; }
    if ($round =~ /^yes$/i)
    {  $round = "yes"; }
    elsif ($round =~ /^no$/i)
    {  $round = "no"; }
    else
    {  $self->setError("Invalid round: $round.  Use 'yes' or 'no'\n"); return undef; }
    if (! $self->isValid(period => $period ) )
    { $self->setError("Invalid period: $period\n"); return undef; }
    if (! $self->isValid(date => $date ) )
    { $self->setError("Invalid date '$date'"); return undef; }
    else
    {  $foundDate = $self->lookupDate(date=> $date);       }
    if (not defined $foundDate)
    {  $self->setError("Invalid found date: $date\n"); return undef; }
    #Step 1.1
...
sub annualize
{
        ...
        my $errStr = "Payroll::US::MO::StateIncome->annualize()  - Error!\n";
        if ( $self->isValid(amount => $args{amount} ) )
        {
                $self->setError($self->{errorString} . $errStr . "Invalid amount: $args{amount}\n");
                return undef;
        }
        if ($args{amount} =~ /^(0(\.0)?)$/)
        {  return 0;  }
        if (not exists $self->{periodDays}->{$args{period}})
        {  $self->setError($errStr . "Invalid period: '$args{period}'\n"); return undef; }
        else
        {
...
sub lookupDate
{
  ...
  if ( $args{date} !~ /^\d{8}$/)
  {
    $self->setError("invalid date format");
    return undef;
  }
