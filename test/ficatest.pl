...
# $Id: ficatest.pl,v 1.1 2003/06/25 13:47:29 moreejt Exp $

use strict;
use Payroll::US::FICA;

my @testData;
my $FICA = Payroll::US::FICA->new(debug => "yes");
...
else
{
 ...
 $FICA->setError("Error:  is set");
 print $FICA->errorMessage() . "\n";
 $FICA->{errorString} = "";
 $FICA->setError(errorString => "Error2: set");
 print $FICA->errorMessage() . "\n";
